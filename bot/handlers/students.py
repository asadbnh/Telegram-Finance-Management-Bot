...
async def start_add_student(callback: CallbackQuery, user_role: UserRole, state: FSMContext):
    ...
    await callback.message.edit_text("👨‍🎓 <b>إضافة طالب جديد</b>\n\nأدخل <b>اسم الطالب الرباعي</b>:", reply_markup=get_back_keyboard(), parse_mode="HTML")
...
async def process_student_name(message: Message, state: FSMContext):
    ...
    await message.answer("🔢 أدخل <b>الرقم الأكاديمي / الكود الخاص بالطالب</b>:", reply_markup=get_back_keyboard(), parse_mode="HTML")
...
async def process_student_phone(message: Message, state: FSMContext):
    ...
    await message.answer("💵 أدخل <b>المبلغ الإجمالي المطلوب من الطالب</b> بالريال (مثال: 100000):", reply_markup=get_back_keyboard(), parse_mode="HTML")
...
async def process_student_total(message: Message, state: FSMContext, db_user: User, db_session: AsyncSession):
    ...
    success_msg = (
        f"✅ <b>تمت إضافة الطالب بنجاح!</b>\n\n"
        f"👤 <b>الاسم:</b> {student.name}\n"
        f"🔢 <b>الرقم:</b> {student.student_code}\n"
        f"💵 <b>المبلغ المطلوب:</b> {format_currency(student.total_required)}\n"
    )
...
async def start_record_fee(callback: CallbackQuery, user_role: UserRole, state: FSMContext):
    ...
    await callback.message.edit_text("🔍 <b>تسجيل رسوم طالب</b>\n\nاكتب <b>اسم الطالب</b> أو <b>رقمه الأكاديمي</b> للبحث عنه:", reply_markup=get_back_keyboard(), parse_mode="HTML")
    await callback.answer()
...
async def process_student_search(message: Message, state: FSMContext, db_session: AsyncSession):
    ...
    await message.answer("👇 <b>اختر الطالب من القائمة التالية:</b>", reply_markup=kb, parse_mode="HTML")
...
async def process_select_student(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("💵 أدخل <b>المبلغ المدفوع</b> بالريال:", reply_markup=get_back_keyboard(), parse_mode="HTML")
    await callback.answer()
...
async def process_fee_amount(message: Message, state: FSMContext):
    ...
    await message.answer("👤 أدخل <b>اسم المستلم</b> (المسؤول المالي الذي استلم الدفعة):", reply_markup=get_back_keyboard(), parse_mode="HTML")
...
async def process_fee_notes(message: Message, state: FSMContext):
    ...
    await message.answer("📷 أرسل <b>صورة السند</b> (أو 'تخطي'):", reply_markup=get_back_keyboard(), parse_mode="HTML")
...
async def process_fee_photo(message: Message, state: FSMContext, bot: Bot, db_user: User, db_session: AsyncSession):
    ...
    success_msg = (
        f"✅ <b>تم تسجيل دفع الرسوم بنجاح!</b>\n\n"
        f"👨‍🎓 <b>الطالب:</b> {student.name}\n"
        f"💰 <b>المبلغ المدفوع:</b> {format_currency(income.amount)}\n"
        f"📊 <b>إجمالي المدفوع حتى الآن:</b> {format_currency(student.total_paid)}\n"
        f"🔴 <b>المتبقي على الطالب:</b> {format_currency(student.remaining_amount)}\n"
...
async def cb_student_statement(callback: CallbackQuery, db_session: AsyncSession):
    ...
    text = f"📜 <b>كشف حساب رسوم الطلاب ({len(students)} طالب):</b>\n\n"
    text += f"✅ <b>عدد المسددين بالكامل:</b> {stats['paid_count']}\n"
    text += f"🔴 <b>عدد المتبقي عليهم:</b> {stats['unpaid_count']}\n"
    text += f"💰 <b>إجمالي المتبقي على الدفعة:</b> {format_currency(stats['total_remaining'])}\n"
    text += "ــــــــــــــــــــــــــــــــــــــــ\n\n"

    for st in students[:15]:
        status_icon = "✅" if st.status.value == "PAID" else "🔴"
        text += (
            f"{status_icon} <b>{st.name}</b> (<code>{st.student_code}</code>)\n"
